...
export class PreOpComponent implements OnInit {
  ...
  tableHeaders = AdvocateReportsTableUtils.preOpTableHeaders;
  ...
  ngOnInit(): void {
    ...
    this.reportFilters.lastContactFrom.value = currentDate.subtract(6, "months").toDate();
  ...
  getReports(){
    this.advocateReportService.getPreOp(this.pagination, this.reportFilters).subscribe({
      next: (response: any) => {
  ...
  exportExcel(){
    if (this.allReportsList && this.allReportsList.length > 0){
      const excelResults = this.allReportsList.map((preOp: any) => {
        return {
          "Name": preOp.patientName,
          "Surgeon": preOp.surgeonName,
          "Insurance": preOp.insuranceCompany,
          "Last Contact": preOp.lastContactString,
          "SubStatus": preOp.subStatus,
          "Advocate": preOp.advocateName,
          "Procedure Name": preOp.procedureName,
          "Tentative Date": preOp.tentativeDateString
        }
      });
      this.excelHelper.exportToExcel(excelResults, 'Pre-Op Report', this.tableHeaders.filter(x => x.name !== 'Action').map(x => x.name));
    }
  }

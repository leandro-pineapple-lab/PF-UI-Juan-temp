...
export class SeminarComponent implements OnInit {
  seminarFilter: EventFilterModel = new EventFilterModel("S");
  newEvent: EventModel = new EventModel("S");
  selectedEvent: EventModel = new EventModel("S");
  ...
  showSeminarTitleError: boolean = false;
  closeResult: string = "";
  ...
  addEventForm = new UntypedFormGroup({
    ...
    addEventSubType: new UntypedFormControl(""),
  });
  ...
  get addEventSubType()
  {
    return this.addEventForm.get("addEventSubType");
  }
  ...
  updateEventForm = new UntypedFormGroup({
    ...
    updateEventSubType: new UntypedFormControl(""),
  });
  ...
  get updateEventSubType()
  {
    return this.updateEventForm.get("updateEventSubType");
  }
  ...
  async getSeminars(){
    (await this.eventService.getEvents(this.seminarFilter)).subscribe({
      next: (response: any) => {
  ...
  async addSeminar(){
    ...
    (await this.eventService.addEvent(this.newEvent)).subscribe({
      next: (response: any) => {
        this.getSeminars();
        this.getPresenters();
        this.toastr.success("Seminar successfully created", 'Success');
  ...
  async updateSeminar(){
    ...
    (await this.eventService.updateEvent(this.selectedEvent)).subscribe({
      next: (response: any) => {
        this.getSeminars();
        this.getPresenters();
        this.toastr.success(this.selectedEvent.id === 0 ? "Seminar cloned successfully" : "Seminar successfully updated", 'Success');
  ...
  openUpdateSeminarConfirmationModal(seminarToUpdateModal: any, seminar: EventModel, clone: boolean = false) {
    this.selectedEvent = new EventModel("S");
    this.selectedEvent.id = !clone ? seminar.id : 0;
    this.selectedEvent.eventTitle = seminar.eventTitle;
    this.selectedEvent.eventStatus = seminar.eventStatus;
    this.selectedEvent.eventDate = seminar.eventDate;
    this.selectedEvent.eventDuration = seminar.eventDuration;
    this.selectedEvent.eventPresenter = seminar.eventPresenter;
    this.selectedEvent.eventLocation = seminar.eventLocation;
    this.selectedEvent.eventAddress1 = seminar.eventAddress1;
    this.selectedEvent.eventAddress2 = seminar.eventAddress2;
    this.selectedEvent.eventCity = seminar.eventCity;
    this.selectedEvent.eventState = seminar.eventState;
    this.selectedEvent.eventZip = seminar.eventZip;
    this.selectedEvent.eventCapacity = seminar.eventCapacity;
    this.selectedEvent.eventPhone = seminar.eventPhone;

    this.modalService.open(seminarToUpdateModal, {ariaLabelledBy: 'modal-basic-title', size: 'lg'}).result.then((result) => {
    ...
    (await this.eventService.getEvents(this.seminarFilter)).subscribe({
      next: (response: any) => {
        if (response.object != null && response.object.length > 0) {
          const seminars = response.object.map((seminar: EventModel) => {
            return {
              "Date": this.datePipe.transform(seminar.eventDate, 'M/d/yyyy h:mm a'),
              "Title": seminar.eventTitle,
              "Location": seminar.eventLocation,
              "City": seminar.eventCity,
              "Presenter": seminar.eventPresenter,
              "Capacity": seminar.eventCapacity,
              "Status": seminar.eventStatus === "O" ? "Open" : seminar.eventStatus === "C" ? "Closed" : "Cancelled"
            }
          });
          this.excelHelper.exportToExcel(seminars, 'Settings Events', ["Date", "Title", "Location", "City", "Presenter", "Capacity", "Status"]);
        }
      },
      error: (e: any) => {
        this.toastr.error(e.error, 'Error');
